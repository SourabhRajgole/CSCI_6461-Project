...
/**
 * MachineSimulator is a graphical simulator for a basic machine architecture.
 * This program provides a GUI interface with various registers and memory
 * handling for executing and simulating low-level machine instructions.
 */
...
// Index Registers (IXR)
	...
	private void createUI() {
		...
		// -- Cache Content Area (Moved to x=4, anchored top-left, with a negative top
		// inset) --
...
// Add all buttons to the buttonPanel (IPL is included here, but its color is already set)
		JButton[] buttons = { loadButton, storeButton, loadPlusButton, storePlusButton, runButton, stepButton,
				haltButton, iplButton };
...
// Add button panel to main panel
	...
	private void displayLoadedROMContents() {
		...
		for (int i = 0; i < MEMORY_SIZE; i++) {
			if (memory[i] != 0) {
				...
				printerArea.append("Address (Decimal): " + decimalAddress + ", Instruction (Binary): "
						+ instructionInBinary + "\n");
			}
		}
	}
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				if (address >= 0 && address < MEMORY_SIZE && gprIndex >= 0 && gprIndex < 4) {
					...
					printerArea.append("LDR: Loaded value " + memory[address] + " from memory address " + address
							+ " into GPR " + gprIndex + ".\n");
				} else {
	...
	private class StoreActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				int address = Integer.parseInt(marField.getText()); // Get the memory address from MAR
				// int gprIndex = Integer.parseInt(octalInputField.getText()); // Get the GPR
				// index from octalInputField
